...
export const financeApi = {
  ...
  getAdjustmentCategories: (type) => api.get('/finance/adjustment-categories', type ? { type } : undefined).then(extractData).catch(handleError),

  createAdjustmentCategory: (data) => api.post('/finance/adjustment-categories', data).then(extractData).catch(handleError),

  // Journal Entries

  getJournalEntries: () => api.get(`/finance/journal-entries?_t=${Date.now()}`).then(extractData).catch(handleError),
