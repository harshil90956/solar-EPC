...
let _CommissioningTasksCache = [];
export const setTasksCache = (tasks) => { _CommissioningTasksCache = tasks || []; };
export const getTasksFromSettings = () => _CommissioningTasksCache || [];
...
const formatEventType = (type) => {
  const labels = {
    ...
    'Commissioning_started': 'Commissioning Started',
...
const formatEventMetadata = (metadata) => {
  ...
  if (metadata.CommissioningId) {
    parts.push(`Commissioning ID: ${metadata.CommissioningId}`);
  }
  if (metadata.by) {
...
const CommissioningCalendar = ({ logs, onOpenCommissioning }) => {
    ...
    <div className="flex gap-4 h-[calc(100vh-280px)] min-h-[480px]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-base)] bg-gradient-to-r from-[var(--primary)]/10 to-transparent">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-[var(--text-muted)] mt-0.5">
              {Object.keys(dayMap).filter(k => k.startsWith(`${calYear}-${String(calMonth+1).padStart(2,'0')}`)).length} Commissionings this month
            </p>
          </div>
          <button
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-[var(--border-base)] bg-gradient-to-r from-[var(--primary)]/10 to-transparent">
          ...
          <p className="text-xs text-[var(--primary)] font-semibold mt-1">{selectedInstalls.length} Commissioning{selectedInstalls.length !== 1 ? 's' : ''}</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {selectedInstalls.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-32 text-[var(--text-muted)]">
              ...
              <p className="text-xs">No Commissionings</p>
            </div>
          ) : selectedInstalls.map((inst, idx) => (
            <button
              key={inst._id || inst.id}
              onClick={() => onOpenCommissioning(inst)}
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card p-4 rounded-2xl border border-[var(--border-base)]">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="text-xs text-[var(--text-muted)] font-medium">
            Showing: <span className="text-[var(--primary)] font-bold">{filteredLogs.length}</span> of <span className="text-[var(--primary)] font-bold">{logs.length}</span> Commissionings
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-6 rounded-2xl border border-[var(--border-base)] animate-fade-in">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-lg font-bold text-[var(--text-primary)]">Recent Commissionings</h3>
            <p className="text-xs text-[var(--text-muted)]">Latest Commissioning activity</p>
          </div>
        </div>
            ...
            <div key={log._id || log.id} className="flex items-center gap-4 p-4 rounded-xl bg-[var(--bg-muted)]/50 hover:bg-[var(--primary)]/5 transition-all cursor-pointer group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  {log.CommissioningId && !/^[0-9a-fA-F]{24}$/.test(log.CommissioningId) && (
                    <span className="text-[10px] font-mono font-bold text-[var(--primary)]">{log.CommissioningId}</span>
                  )}
...
const CommissioningPage = () => {
  ...
  useEffect(() => {
    const checkOverdue = async () => {
      try {
        const result = await apiClient.post('/commissionings/check-overdue');
        if (result.data?.updated > 0) {
          toast.info(`${result.data.updated} Commissioning(s) marked as delayed`);
          refetch();
        }
      } catch (err) {
  ...
  const handleMove = async (id, toStatus) => {
    const log = logs.find(x => x.CommissioningId === id || x.id === id || x._id === id);
    if (!log) return;
    ...
    try {
      await apiClient.patch(`/commissionings/${log._id || log.id}/status`, { status: toStatus });
      await refetch();
      // Update selected state if modal is open for this Commissioning
  ...
  const filtered = useMemo(() => {
    let result = logs.filter(l => {
      ...
      return (l.CommissioningId||'').toString().toLowerCase().includes(s)
  ...
  const handleToggleTask = async (index) => {
    ...
    try {
      ...
      const resp = await apiClient.patch(`/commissionings/${selected._id || selected.id}/tasks`, { tasks: updatedTasks });
  ...
  const handlePhotoFileChange = async (e) => {
    ...
    try {
      ...
      const response = await apiClient.post('/commissionings/upload/task-photos', {
        taskName: currentTaskForPhoto,
        CommissioningId: selected._id || selected.id,
        image: base64 // Send base64 encoded image
      });

      // Update selected Commissioning with new photos
      ...
      if (taskIndex >= 0 && !currentTasks[taskIndex].done) {
        ...
        // Update backend
        await apiClient.patch(`/commissionings/${selected._id || selected.id}/tasks`, {
          tasks: updatedTasks
        });
  ...
  const handleDeletePhoto = async (photo) => {
    ...
    try {
      ...
      const response = await apiClient.delete(`/commissionings/${selected._id || selected.id}/photos/${encodeURIComponent(photo.key)}`);
  ...
  // Calendar data derived from Commissioning events (server stores events array)
  const calendarEvents = useMemo(() => {
    const ev = [];
    logs.forEach(l => {
      (l.events || []).forEach(e => ev.push({ CommissioningId: l.CommissioningId, timestamp: e.timestamp, eventType: e.eventType, metadata: e.metadata }));
      // ensure a created event exists
      if (!l.events || !l.events.some(x=>x.eventType==='Commissioning Created')) {
        ev.push({ CommissioningId: l.CommissioningId, timestamp: l.createdAt || l._created || null, eventType: 'Commissioning Created', metadata: {} });
      }
    });
  ...
  const createCommissioning = async () => {
    ...
    try {
      ...
      console.error('Create Commissioning error:', err);
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader 
        title="Commissioning" 
        subtitle="Site Commissioning logs, checklist and photos" 
      ...
      {view === 'dashboard' && (
        <CommissioningDashboard logs={logs} />
      )}
      
      {view === 'kanban' && (
        <InstallKanbanBoard items={logs} onCardClick={setSelected} onDrop={handleMove} canEdit={can('commissioning', 'edit')} />
      )}
      ...
      {view === 'table' && (
        <DataTable
          ...
          emptyText="No Commissioning logs found."
          onRowClick={(row) => setSelected(row)}
          bulkActions={[
            ...(can('commissioning','export') || can('commissioning','view') ? [{
              label: 'Export',
              icon: Download,
              onClick: (ids) => {
                const rows = ids.length > 0
                  ...
                  : paginated;
                const csv = [
                  ['Commissioning ID','Project','Customer','Site','Technician','Status','Progress','Due Date'],
                  ...rows.map(r => [
                    r.CommissioningId, typeof r.projectId === 'object' ? (r.projectId?.projectId || '-') : (r.projectId || '-'),
                ...
                a.download = 'Commissionings.csv';
                a.click();
                toast.success(`Exported ${rows.length} Commissionings`);
              }
            }] : []),
            ...(can('commissioning','assign') || can('commissioning','edit') ? [{
              ...
              icon: UserMinus,
              onClick: async (ids) => {
                if (!window.confirm(`Unassign technician from ${ids.length} Commissioning(s)?`)) return;
                try {
                  await Promise.all(ids.map(id =>
                    apiClient.patch(`/commissionings/${id}`, {
                      technicianId: null,
                      technicianName: 'Not Assigned',
                      assignedTo: null,
                      department: '',
                  ...
                  refetch();
                  toast.success(`Unassigned ${ids.length} Commissioning(s)`);
          ...
          rowActions={[
            ...
            ...(can('commissioning','assign') || can('commissioning','edit') ? [{
              ...
              onClick: async (row) => {
                if (!window.confirm(`Unassign technician from ${row.CommissioningId}?`)) return;
                try {
                  await apiClient.patch(`/commissionings/${row._id || row.id}`, {
                    technicianId: null,
                    technicianName: 'Not Assigned',
                    assignedTo: null,
                    department: '',
                  ...
                  refetch();
                  toast.success(`Unassigned from ${row.CommissioningId}`);
                } catch(err) { toast.error(err.message || 'Unassign failed'); }
              }
            }] : []),
      ...
      {view === 'calendar' && (
        <CommissioningCalendar logs={logs} onOpenCommissioning={setSelected} />
      )}
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  setSelectedRows(new Set());
                  refetch();
                  toast.success(`Assigned ${bulkAssignIds.length} Commissioning(s) to ${bulkAssignForm.technicianName}`);
      ...
      >
        <div className="space-y-4 py-1">
          ...
          {/* Selected Commissionings preview */}
          <div className="rounded-lg bg-[var(--bg-muted,var(--bg-surface))] border border-[var(--border-base)] p-3">
            <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-2">Selected Commissionings</p>
        ...
        <Modal open={!!selected} onClose={()=>setSelected(null)} title={`Commissioning — ${selected.CommissioningId || selected.id}`} footer={
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="glass-card p-2"><div className="text-[var(--text-muted)]">Commissioning ID</div><div className="font-semibold">{selected.CommissioningId || selected.id}</div></div>
            ...
            <div>
              ...
              <div className="space-y-2 mt-2">
                {(mergeWithSettingsTasks(selected.tasks) || []).map((t,i)=> {
                  ...
                  const hasAssignedScope = user?.dataScope === 'ASSIGNED' || user?._doc?.dataScope === 'ASSIGNED';
                  const isEmployeeRole = user?.role?.toLowerCase() === 'employee' || user?.role?.toLowerCase() === 'technician';
                  const canUpdateTask = can('commissioning','edit') || isAssignedUser || hasAssignedScope || isEmployeeRole;
                ...
                {(() => {
                  ...
                  const hasAssignedScope = user?.dataScope === 'ASSIGNED' || user?._doc?.dataScope === 'ASSIGNED';
                  const isEmployeeRole = user?.role?.toLowerCase() === 'employee' || user?.role?.toLowerCase() === 'technician';
                  const canComplete = can('commissioning','edit') || isAssignedUser || hasAssignedScope || isEmployeeRole;
                  ...
                  {selected.photos.map((photo, idx) => (
                    <div key={idx} className="relative group cursor-pointer overflow-hidden rounded-lg border border-[var(--border-base)] bg-[var(--bg-surface)]">
                      ...
                      <div className="relative w-full pt-[75%]">
                        <img
                          src={photo.url}
                          alt={photo.taskName || 'Commissioning photo'}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center min-h-[200px]">
                      <img
                        ...
                        alt={photoModal.photo.taskName || 'Commissioning photo'}
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button variant="primary" onClick={async ()=>{
                try {
                  ...
                  await apiClient.patch(`/commissionings/${editForm._id || editForm.id}`, cleanData);
                  toast.success('Commissioning updated');
                  setShowEdit(false);
                  setEditForm(null);
                  refetch();
                } catch (err) {
